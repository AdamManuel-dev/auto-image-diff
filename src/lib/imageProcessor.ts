...
export class ImageProcessor {
  ...
  async alignImages(
    ...
    return new Promise((resolve, reject) => {
      imageMagick(referenceImage)
        ...
        }, (err: any, _isEqual: any, _equality: any, raw: any) => {
          if (err && !raw.includes('@ ')) {
            reject(err);
            return;
          }
  ...
  async compareImages(
    ...
    return new Promise((resolve, reject) => {
      imageMagick(image1Path)
        .compare(image2Path, { metric: 'AE' }, (err: any, _isEqual: any, equality: any, raw: any) => {
          if (err && !raw) {
            reject(err);
            return;
          }

          const pixelsDifferent = parseInt(raw || '0', 10);
    ...
    return new Promise((resolve, reject) => {
      ...
      imageMagick(image1Path)
        .compare(image2Path, {
          ...
          highlightColor,
          lowlight
        }, async (err: any, _isEqual: any, _equality: any, _raw: any) => {
