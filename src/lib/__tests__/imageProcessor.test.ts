  ...
  const gmMock = {
    subClass: jest.fn(() => {
      const imageMagick = jest.fn(() => ({
        compare: jest.fn((_targetImage, options, callback) => {
        ...
        geometry: jest.fn(() => ({
          write: jest.fn((_outputPath, callback) => {
            callback(null);
          })
        })),
        write: jest.fn((_outputPath, callback) => {
...
describe('ImageProcessor', () => {
  ...
  describe('alignImages', () => {
    it('should align images with detected offset', async () => {
      ...
      await expect(processor.alignImages(reference, target, output))
