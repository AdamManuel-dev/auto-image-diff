...
program
  ...
  .action(async (reference, target, output, options) => {
    try {
      ...
      console.error('❌ Error aligning images:', error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (image1, image2, output, options) => {
    try {
      ...
      console.error('❌ Error generating diff:', error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
...
program
  ...
  .option('-c, --color <color>', 'Highlight color for differences', 'red')
  .action(async (reference, target, outputDir, options) => {
    try {
      ...
      console.error('❌ Error in comparison:', error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
